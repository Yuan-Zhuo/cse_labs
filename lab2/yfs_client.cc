...
int yfs_client::create(inum parent, const char *name, mode_t mode,
  ...
  bool found_lookup;
  if ((lookup_internal(parent, name, found_lookup, ino_out) !=
       extent_protocol::OK) ||
      found_lookup) {
    r = EXIST;
    goto release;
...
// internal
int yfs_client::lookup_internal(inum parent, const char *name, bool &found,
                                inum &ino_out) {
  int r = OK;
  assert(name);

  std::list<dirent> ent_list;
  if (readdir_internal(parent, ent_list) != extent_protocol::OK) {
    r = IOERR;
    goto release;
  }

  found = false;
  for (std::list<dirent>::iterator it = ent_list.begin(); it != ent_list.end();
       ++it) {
...
// internal
int yfs_client::readdir_internal(inum dir, std::list<dirent> &list) {
  int r = OK;
...
int yfs_client::unlink(inum parent, const char *name) {
  ...
  if ((ec->remove(ino) != extent_protocol::OK) ||
      (ec->put(parent, dir_buf) != extent_protocol::OK)) {
    r = IOERR;
    goto release;
  }
...
int yfs_client::symlink(inum parent, const char *name, const char *link,
  ...
  if ((ec->create(extent_protocol::T_SYMLINK, ino_out) !=
       extent_protocol::OK) ||
      ec->put(ino_out, std::string(link)) != extent_protocol::OK) {
    r = IOERR;
    goto release;
  }
